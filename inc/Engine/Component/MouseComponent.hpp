...
namespace orbit {
	...
	// @endexample
	class MouseComponent : public Component, public EventDriven
	{
	public:
	...
	private:
		...
		Ptr<IDirectInputDevice8W> _mouseDevice;
	...
	public:
		...
		MouseComponent(ObjectPtr boundObject);
		// @brief: create a new Keyboard Handler
		// @param input: the direct input device
		// @param hWnd: the window handle
		// @return: a shared_ptr to the newly created Keyboard Handler
		static std::shared_ptr<MouseComponent> create(ObjectPtr boundObject);
