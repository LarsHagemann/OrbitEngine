...
namespace orbit
{
	...
	public:
		...
		// @brief: returns the target to follow
		TransformPtr GetTarget() const { return _target; }
		// @brief: sets the minimum and maximum distance to the target
		...
		// @brief: sets the distance to the target (clamped to the min/max distance)
		// @param distance: the new distance to the target
		void SetDistance(float distance);
		// @brief: sets the offset from the target position
		// @param offset: the offset from the target
		void SetTargetOffet(const Vector3f& offset);
		// @brief: tilts the camera
