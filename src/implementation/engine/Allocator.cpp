...
namespace orbit
{
	...
	{
		ORBIT_INFO_LEVEL(ORBIT_LEVEL_DEBUG, "Creating new allocator page [%d]", _pagesize);
	...
	Allocator::Allocator(size_t pagesize) :
		_pagesize(pagesize)
	{
		ORBIT_INFO_LEVEL(ORBIT_LEVEL_DEBUG, "Initializing Allocator with pagesize <%d>", pagesize);
	}
